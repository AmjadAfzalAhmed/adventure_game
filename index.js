...
async function initialize() {
    ...
    const spinner = createSpinner("In order to continue playing, first, you need to have a potion, as a player fuel").start();
    console.log("\n");
...
async function promptForPotion() {
    ...
    if (potionAnswer.selectPotion === "Grapes") {
        console.log("\t\n Your fuel level is 70%", chalk.green.inverse("---------------"));
    }
    else if (potionAnswer.selectPotion === "Apple") {
        console.log("\t\n Your fuel level is 80%", chalk.magenta.inverse("----------------"));
    }
    else if (potionAnswer.selectPotion === "WaterMelon") {
        console.log("\t\n Your fuel level is 90%", chalk.red.inverse("----------------------"));
    }
    else if (potionAnswer.selectPotion === "Pomegranate") {
        console.log("\t\n Your fuel level is 100%", chalk.yellow.inverse("-------------------------"));
    }
}
async function selectOpponent() {
    ...
    if (opponent.selectOppo === "Loki") {
        ...
        console.log(chalk.blue.inverse("\t You selected Loki as an Opponent"), chalk.yellow.italic("\t\n remember Loki is a mischievous opponent"));
    }
        ...
        player2 = opponent.selectOppo;
        console.log(chalk.cyanBright.inverse("\t You selected Ultron as Opponent"), chalk.yellow.italic(chalk.redBright.italic("\t\n Ultron is a high tech robotic machine, you must be witty and quick to defeat him!")));
    }
}
